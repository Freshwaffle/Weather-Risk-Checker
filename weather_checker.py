...
def fetch_and_analyze():
    ...
    try:
        ...
        for i in range(len(data['time'])):
            ...
            # Hourly alerts - only if CAPE AND shear both strong
            if cape >= 500 and (shear_03 >= 20 or deep_shear >= 30):
                alert_flags = []
                if cape >= 1000: alert_flags.append(f"Moderate Instability (CAPE {cape:.0f})")
                if shear_03 >= 20: alert_flags.append(f"Strong 0–3 km Shear ({shear_03:.0f} kt)")
                if deep_shear >= 30: alert_flags.append(f"Strong Deep-Layer Shear ({deep_shear:.0f} kt)")
                if precip >= 0.5: alert_flags.append(f"Precipitation: {precip:.2f} in")

                alerts.append(f"{time_str}: {', '.join(alert_flags)}")
        ...
        for i in range(min(24, len(data['time']))):
            if (data['wind_direction_10m'][i] is None or 
                data['wind_direction_700hPa'][i] is None or
                data['wind_direction_300hPa'][i] is None):
                continue
        ...
        with result_container:
            ...
            if alerts:
                ui.label(f"{len(alerts)} potential setup hours detected:").classes("text-xl text-red-600")
                for a in alerts:
                    ui.label(a)
            else:
