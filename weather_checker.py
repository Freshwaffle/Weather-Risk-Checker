...
def fetch_and_analyze():
    url = (
        ...
        f"hourly=cape,cin,wind_speed_10m,wind_direction_10m,precipitation&"
    ...
    try:
        ...
        for i in range(len(data['time'])):
            ...
            if spd_sfc >= 20:
                flags.append(f"Strong Surface Winds ({spd_sfc:.0f} kt)")
            if precip >= 0.75:
                flags.append(f"Heavy Rain ({precip:.2f} in)")
            if cin >= 50:
                flags.append(f"Strong CIN ({cin:.0f}) — Inhibition")

            if len(flags) >= 2:
                alerts.append(f"{time_str}: {', '.join(flags)}")

            # Store for CSV or further processing
            hourly_data_for_csv.append({
                "time": time_str,
                "CAPE": cape,
                "CIN": cin,
                "wind_kt": spd_sfc,
        ...
        avg_wind = sum((data['wind_speed_10m'][i] or 0) * KMH_TO_KT for i in range(min(24,len(data['time'])))) / min(24,len(data['time']))
        ...
        # ===== Fail Modes =====
        fail_modes = []
        if max_cape < 500 and avg_wind >= 25:
            fail_modes.append("Strong shear but weak instability — possible elevated storms")
        if max_cape >= 1500 and avg_wind < 10:
            fail_modes.append("High CAPE but low wind — limited severe potential")
        ...
        with result_container:
            ...
            ui.label(
                ...
                f"Avg Surface Wind: {avg_wind:.1f} kt | "
                f"Total Precip: {total_precip:.2f} in"
            ).classes("text-lg mb-4")

            if fail_modes:
                ui.label("Potential Fail Modes:").classes("text-lg font-semibold text-red-600")
                for f in fail_modes:
                    ui.label(f"- {f}")
            ...
            # CSV download
            def download_csv():
                filename = f"weather_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
                with open(filename, 'w', newline='') as csvfile:
                    writer = csv.DictWriter(csvfile, fieldnames=hourly_data_for_csv[0].keys())
                    writer.writeheader()
                    writer.writerows(hourly_data_for_csv)
                ui.notify(f"CSV saved: {filename}")

            ui.button("Download Hourly CSV", on_click=download_csv).classes("mt-4")

    except Exception as e:
        result_container.clear()
        ui.label(f"Error fetching weather: {e}").classes("text-red-600")
