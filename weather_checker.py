...
def fetch_and_analyze():
    ...
    try:
        ...
        alerts_full = []   # CAPE + shear
        alerts_kin = []    # Shear only, CAPE < threshold
        ...
        for i in range(len(data['time'])):
            ...
            flags = []
            if cape >= 500:
                flags.append(f"CAPE {cape:.0f} J/kg")
            if shear_03 >= 25:
                flags.append(f"0–3 km Shear {shear_03:.0f} kt")
            if deep_shear >= 35:
                flags.append(f"0–6 km Shear {deep_shear:.0f} kt")
            if precip >= 0.75:
                flags.append(f"Heavy Rain {precip:.2f} in")

            # Decide type of alert
            if cape >= 500 and (shear_03 >= 25 or deep_shear >= 35):
                alerts_full.append(f"{time_str}: {', '.join(flags)}")
            elif cape < 500 and (shear_03 >= 25 or deep_shear >= 35):
        ...
        for i in range(min(24, len(data['time']))):
            if data['wind_direction_10m'][i] is None or data['wind_direction_700hPa'][i] is None or data['wind_direction_300hPa'][i] is None:
                continue
            ...
            deep_shear_list.append(((u_300 - u_sfc)**2 + (v_300 - v_sfc)**2)**0.5)

        avg_shear_03 = sum(shear_03_list)/len(shear_03_list) if shear_03_list else 0
        avg_deep_shear = sum(deep_shear_list)/len(deep_shear_list) if deep_shear_list else 0
        ...
        with result_container:
            ...
            # Full convective alerts
            if alerts_full:
                ui.label(f"{len(alerts_full)} Convective Setup Hours Detected:").classes("text-xl text-red-600")
                for a in alerts_full:
                    ui.label(a)
            # Kinematic-only alerts
            if alerts_kin:
                ui.label(f"{len(alerts_kin)} Kinematic Setup Hours (CAPE < 500 J/kg):").classes("text-xl text-yellow-600")
                for a in alerts_kin:
                    ui.label(a)

            if not alerts_full and not alerts_kin:
